...
-- 1. Tabela chat_business_hour_overrides (ausências/feriados)
CREATE TABLE public.chat_business_hour_overrides (
...
ALTER TABLE public.chat_business_hour_overrides ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Tenant members manage overrides"
  ON public.chat_business_hour_overrides FOR ALL
  USING (tenant_id = get_user_tenant_id(auth.uid()))
  WITH CHECK (tenant_id = get_user_tenant_id(auth.uid()));

-- 2. Tabela chat_business_hour_breaks (intervalos)
CREATE TABLE public.chat_business_hour_breaks (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  business_hour_id uuid NOT NULL REFERENCES public.chat_business_hours(id) ON DELETE CASCADE,
  tenant_id uuid REFERENCES public.tenants(id) ON DELETE CASCADE,
...
ALTER TABLE public.chat_business_hour_breaks ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Tenant members manage breaks"
  ON public.chat_business_hour_breaks FOR ALL
  USING (tenant_id = get_user_tenant_id(auth.uid()))
  WITH CHECK (tenant_id = get_user_tenant_id(auth.uid()));

-- 3. Coluna allow_reopen_resolved em chat_settings
ALTER TABLE public.chat_settings
  ADD COLUMN allow_reopen_resolved boolean NOT NULL DEFAULT false;

-- 4. Coluna previous_status em attendant_profiles
ALTER TABLE public.attendant_profiles
  ADD COLUMN previous_status text;

-- 5. Atualizar trigger assign_chat_room para verificar overrides e breaks
...
BEGIN
  ...
  IF FOUND THEN
    IF v_override.is_closed THEN
      RETURN NEW; -- closed for the day
    END IF;
    -- Special hours: check if within the override window
    IF v_override.start_time IS NOT NULL AND v_override.end_time IS NOT NULL THEN
      ...
      END IF;
    END IF;
    -- Override found and within hours, skip normal business hours check
  ELSE
