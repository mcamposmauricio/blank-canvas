...
CREATE INDEX IF NOT EXISTS idx_contacts_active_companies ON public.contacts(tenant_id) WHERE is_company = true AND is_active = true;
CREATE INDEX IF NOT EXISTS idx_contacts_category_tenant ON public.contacts(tenant_id, service_category_id) WHERE is_company = true AND is_active = true;

-- 3. Update assign_chat_room to skip inactive companies
...
BEGIN
  ...
  SELECT service_category_id, service_priority, is_active
  INTO v_contact
  ...
  -- Skip assignment for inactive companies
